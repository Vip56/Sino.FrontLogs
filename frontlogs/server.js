...
app.use((req, res, next) => {
    res.header('Access-Control-Allow-Origin', 'http://comm.sowl.cn');
            ...
            switch (hostname) {
                case "tms.sowl.cn":
                    ...
                    pool.getConnection((err, conn) => {
                        ...
                        conn.query('INSERT INTO tms_log_total VALUES(NULL,?,?,?,?,?,?)', [level, time, uid, url, userAgent, msgs], (err, result) => {
                            if (err) {
                                return;
                            }
                            conn.release();
                ...
                case "csp.sowl.cn":
                    ...
                    pool.getConnection((err, conn) => {
                        ...
                        conn.query('INSERT INTO csp_log_total VALUES(NULL,?,?,?,?,?,?)', [level, time, uid, url, userAgent, msgs], (err, result) => {
                            if (err) {
                                return;
                            }
                            conn.release();
                ...
                case "ccp.sowl.cn":
                    ...
                    pool.getConnection((err, conn) => {
                        ...
                        conn.query('INSERT INTO ccp_log_total VALUES(NULL,?,?,?,?,?,?)', [level, time, uid, url, userAgent, msgs], (err, result) => {
                            if (err) {
                                return;
                            }
                            conn.release();
